...
The RPM spec file provided here is for Pizcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://pizcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Pizcoin git repository](https://github.com/pizcoin/pizcoin) but are not part of
...
Currently only `Source10` is used, for the example `pizcoin.conf` file.
...
in the pizcoin git repository. At present only an SVG version of the Pizcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the pizcoin
...
needed when 0.12.1 is released, a proper fix is already in the Pizcoin git
master branch.
...
idea to build Pizcoin Core against a static version of that library compiled
...
The `pizcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `pizcoin-server` package. At this point, most RPM based
...
The `pizcoin-server` package should have SELinux support. How to properly do
...
Pizcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/pizcoin/). He has been
packaging Pizcoin for Fedora at least since 2012.
...
4. I renamed the `pizcoin` package that contains the Qt GUI to `pizcoin-core` as
that appears to be how the general population refers to it, in contrast to
`pizcoin-xt` or `pizcoin-classic`. I wanted to make sure the general population
