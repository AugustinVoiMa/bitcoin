...
###   pizcoincore.org and pizcoin.org and compares them.
...
WORKINGDIR="/tmp/pizcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://pizcoincore.org"
HOST2="https://pizcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="pizcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'pizcoin-',
...
if [ $? -ne 0 ]; then
   echo "pizcoin.org failed to provide signature file, but pizcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "pizcoin.org and pizcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Pizcoin Core binary release signing key installed?"
   fi
