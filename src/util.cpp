...
const char * const PIZCOIN_CONF_FILENAME = "pizcoin.conf";
const char * const PIZCOIN_PID_FILENAME = "pizcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "pizcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Pizcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Pizcoin
    // Mac: ~/Library/Application Support/Pizcoin
    // Unix: ~/.pizcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pizcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Pizcoin";
#else
    // Unix
    return pathRet / ".pizcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", PIZCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", PIZCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Pizcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Pizcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Pizcoin Core developers";
    }
